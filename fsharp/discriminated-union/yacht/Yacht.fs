...
let score (category: Category) (dice: Die list): int = 
    ...
    let filterDie (dice: Die list) (die: Die): Die list = List.filter (fun d -> d = die) dice
    let sumDice (dice: Die list): int = List.sumBy (fun die -> int die) dice
    let calcSingles (dice: Die list) (die: Die): int =  List.length (filterDie dice die) * int die
    let calcFullHouse dice = 
        let group = List.groupBy (fun die -> die) dice |> List.sortByDescending (fun (_, dices) -> dices.Length)
    ...
    let calcFourForAKing dice =
        List.groupBy (fun die -> die) dice 
        |> List.sortByDescending (fun (die, _) -> die)
        |> List.head
        |> fun (die, dice) ->  
    ...
    | Choice -> sumDice dice
    | Yacht -> if List.groupBy (fun die -> die) dice |> List.length = 1 then 50 else 0